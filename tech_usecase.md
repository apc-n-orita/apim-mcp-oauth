...
>
> Official MCPs run their own authorization server in a different trust domain from the enterprise IdP. Identity Assertion JWT Authorization Grant (ID-JAG) — see [this blog post on ID-JAG](https://techblog.ap-com.co.jp/entry/2026/09/14/235649) — and its MCP extension, Enterprise-Managed Authorization, are designed exactly for that cross-domain case, and are the effective way to authorize official MCPs: the enterprise IdP becomes the policy decision point for every official MCP server the organization trusts, and revocation is centralized at the IdP instead of scattered across each service.
>
